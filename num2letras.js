function numxletras() {
    ...
    for (let index = 0; index < passs; index++) {
		numero = parseInt(num);
    	centenas=(numero%1000-numero%100); // centenas=(numero%1000-numero%100)/100;
    	decenas=(numero%100-numero%10); // decenas=(numero%100-numero%10)/10;
    	unidades=numero%10;
        numx=decenas+unidades;
        numxx=centenas+decenas+unidades;

    if (num >= 100) {if (num == 100) { enletras += 'cien '; }
              else { enletras += cen[centenas] + ' '; }}
    ...
    if (index == 1 && numx == 0 && centenas == 0) { enletras += ''; } else {
    enletras += (index == 2 && numx == 1) ? " millón " : buc[index];
    
    }

        xxenletras = enletras + xxenletras;
        xenletras = '';
        xenletras = enletras;

        numxxx = (num-numxx)/1000; 
        if (numxxx >= 1) { num = numxxx; console.log('num: '+num); enletras = '';xenletras = ''; }
